...
def get_joint_angles(img: np.ndarray, img2: np.ndarray) -> list:
    ...
    print('red - green ', end='')
    red_green = np.array([xi - xj for xi, xj in zip(red_3d_coords, green_3d_coords)])
    red_green[2] = -red_green[2]
    print(red_green)
...
class RobotControl:
    ...
    def callback2(self, data):
        ...
        self.img_2_processed = True
        if self.img_1_processed:
            self.img_2_processed = False
            self.img_1_processed = False
    ...
    def callback1(self, data):
        ...
        self.img_1_processed = True
        if self.img_2_processed:
            self.img_2_processed = False
            self.img_1_processed = False
            jointanglelist = get_joint_angles(self.cv_image1, self.cv_image2)
            joint_angle = Float64()
