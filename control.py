...
class RobotControl:
    def __init__(self):
        ...
        self.all_joint_angles_sub = rospy.Subscriber(ALL_ANGLES_TOPIC, Float64, self.callback_joint_angles)
        ...
        self.publicator_track_dif = rospy.Publisher(DIF_TOPIC, Float64, queue_size=1)
    ...
    def callback_joint_angles(self, data):
        ...
        pkg = Float64MultiArray()
        pkg.data = self.end_effector - self.target_pos_sub
        self.publicator_track_dif.publish(pkg)

    def callback_move(self, data):
        target_pos = data.data*METER_TO_PIXEL
        inverse_jacobian = np.linalg.pinv(self.jacobian)
        current_time = rospy.get_time()
        dt = current_time - self.t_previous
        self.t_previous = current_time
        self.pos_diff = (target_pos - self.end_effector) / dt
